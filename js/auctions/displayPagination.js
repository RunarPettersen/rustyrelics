...
export function displayPagination({ currentPage, totalPages, lastSearch, lastTag, lastActiveOnly }) {
  ...
  const prevBtn = `<button class="px-4 py-2 rounded bg-gold-500 hover:bg-gold-600 text-white cursor-pointer" ${currentPage === 1 ? "disabled" : ""}
    data-page="${currentPage - 1}">Previous</button>`;

  const nextBtn = `<button class="px-4 py-2 rounded bg-gold-500 hover:bg-gold-600 text-white cursor-pointer" ${currentPage === totalPages ? "disabled" : ""}
